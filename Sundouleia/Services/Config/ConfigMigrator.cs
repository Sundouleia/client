...
public static class ConfigMigrator
{
    public static JObject MigrateMainConfig(JObject mainConfig, ConfigFileProvider fileNames)
    {
        ...
        foreach (var file in Directory.GetFiles(ConfigFileProvider.SundouleiaDirectory, "config-testing.json.bak*"))
        {
...
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
    ...
    public static JObject MigratePatternConfig(JObject oldConfig, ConfigFileProvider fileNames)
    {
        ...
        // move all old files into the backup folder.
        foreach (var file in Directory.GetFiles(ConfigFileProvider.SundouleiaDirectory, "patterns.json.bak*"))
        {
            // Send it to the shadow realm.
            var fileName = Path.GetFileName(file);
            File.Delete(file);
    ...
    public static JObject MigrateNickConfig(JObject nicknamesConfig)
    {
        Svc.Logger.Warning("Outdated NickConfig detected, migrating to new format!");
