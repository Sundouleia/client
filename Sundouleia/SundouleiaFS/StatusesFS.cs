...
public sealed class StatusesFS : CkFileSystem<LociStatus>, IMediatorSubscriber, IHybridSavable, IDisposable
{
    ...
    public StatusesFS(ILogger<StatusesFS> logger, SundouleiaMediator mediator,
    ...
    private void Reload()
    {
        if (Load(new FileInfo(_hybridSaver.FileNames.CKFS_Statuses), _manager.SavedStatuses, StatusToIdentifier, StatusToName))
            _hybridSaver.Save(this);

        _logger.LogDebug($"Reloaded CKFS with {_manager.SavedStatuses.Count} statuses.");
    }
